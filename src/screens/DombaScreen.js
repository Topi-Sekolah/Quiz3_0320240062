...
export default function DombaScreen({ navigation }) {
  ...
  const fetchData = async () => {
    try {
      ...
      // Filter hanya Domba, sort by kandangHewan asc
      const filtered = json
        .filter(h => h.jenisHewan?.toLowerCase() === 'domba')
        .sort((a, b) => (a.kandangHewan || '').localeCompare(b.kandangHewan || ''));
      setData(filtered);
    } catch (e) {
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.title}>Category - Domba</Text>
      </View>
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <Text style={styles.empty}>Tidak ada data Domba</Text>
        }
      />
      ...
      <View style={styles.infoBox}>
        ...
        <Text style={styles.infoText}>
          Domba sort by kandangHewan asc.
        </Text>
      </View>
    </View>
  );
